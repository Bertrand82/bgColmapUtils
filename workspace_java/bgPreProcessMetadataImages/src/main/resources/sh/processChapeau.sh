...
for d in paquet_*/; do
  ...
  # Skip si fused.las existe déjà dans le dossier
  if [ -f "$dossier/fused.las" ]; then
    log "SKIP: dossier=$dossier car fichier $dossier/fused.las déjà présent"
    echo "bg=chapeau step=skip_process_dossier Dossier=$dossier reason=fused_las_exists date=$(date -Is)"
    continue
  fi
  ...
  # Exécute le paquet en capturant stdout+stderr sans casser la boucle si erreur
  if (
