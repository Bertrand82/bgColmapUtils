...
echo "========================================"
echo " COLMAP - image_undistorber"
echo "========================================"
echo
source ./bgInitConfig.sh
mkdir -p "$OUTPUT_DIR"
touch $LOG
echo "================= image_undistorber =====================================================">>$LOG
...
touch $LOG
echo "$(date '+%Y-%m-%d %H:%M:%S') -  start image_undistorter  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" >> $LOG
START_EPOCH=$(date +%s)
...
echo "End image_undistorter: ">>$LOG
END_EPOCH=$(date +%s)
DURATION_SEC=$((END_EPOCH - START_EPOCH))
DURATION_MIN=$(( (DURATION_SEC + 59) / 60 ))  # arrondi à la minute supérieure
echo "DURATION_MN image_undistorter (minute): $DURATION_MIN" >> $LOG
...
echo "Duree moyenne image_undistorter (seconde) : $DUREE_EXTRACTION_MOYENNE" >> $LOG
